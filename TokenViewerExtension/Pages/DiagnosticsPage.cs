...
internal sealed partial class DiagnosticsPage : ListPage
{
    ...
    public override IListItem[] GetItems()
    {
        ...
        items.Add(ConfigRow("DeepSeek API Key", HasApiKey, "查询余额用"));
        items.Add(ConfigRow("DeepSeek 官方平台 Token", _app.Direct.IsConfigured("deepseek-platform"), "查看官方用量用，获取步骤见图表页"));
        ...
        foreach (var platform in _app.Direct.Platforms)
        {
            ...
            // 接口成功但 0 tokens：极可能是字段名变化，提示查看原始响应
        ...
        {
            Title = "配置与数据位置",
            Subtitle = @"%LOCALAPPDATA%\TokenViewerExtension\（settings.json 保存设置）",
        });

        items.Add(new ListItem(new NoOpCommand())
        {
            Title = "为什么全是 0？",
            Subtitle = "官方用量需要配置「DeepSeek 官方平台 Token」（与 API Key 不同），获取步骤见「用量图表」页",
        });
    ...
    private bool HasApiKey => !string.IsNullOrWhiteSpace(_app.DeepSeek.ApiKey);

    private static ListItem ConfigRow(string name, bool configured, string hint)
    {
        return new ListItem(new NoOpCommand())
        {
            Title = configured ? $"已配置：{name}" : $"未配置：{name}",
            Subtitle = hint,
        };
    }
        ...
        _ = Task.Run(async () =>
        {
            ...
            _platformTests[platform.Id] = snapshot is null
                ...
                : snapshot.Error is not null
                    ? $"拉取失败：{snapshot.Error}{(snapshot.RawResponse is not null ? $" | 原始响应：{snapshot.RawResponse}" : string.Empty)}"
