...
- 数据来源：token 消耗来自 DeepSeek 官方平台（约 5 分钟延迟），配置平台 Token 后自动同步，无需客户端走代理
- 官方数据包含：按天/按模型用量、输入/输出/缓存 token、请求次数、消费金额（可用时）、账户余额
- 图表页展示每日用量柱状图与模型占比；按天数据官方尚未更新时，先显示月度汇总
- 诊断页的「测试转发」可验证代理链路（可选功能），「测试官方接口」可验证 Token 是否有效

### 关于代理模式（可选）

扩展还提供本地代理功能：把 AI 客户端的 Base URL 改为 http://127.0.0.1:8788/v1 即可实时记账。代理进程随命令面板运行，不适用于后台运行的 agent 程序（如 ZCode）。仅使用官方数据的话，可以完全忽略此功能。

## 常见问题

为什么显示 0？

token 消耗只有两个来源：① 官方平台数据（推荐，需配置平台 Token）；② 本地代理记账（需客户端走代理）。两者都没有配置或没有流量时，用量自然为 0。打开「用量诊断」页即可看到各数据源的配置状态。

数据保存在哪里？

配置和用量数据保存在本机 %LOCALAPPDATA%\TokenViewerExtension\ 目录下（settings.json 为配置，usage.json 为代理统计），不会上传到任何服务器。
...
- 代理模式下，流式响应未返回 usage 时按字符数估算并标注
- 消费金额与费用为估算值，仅供参考
